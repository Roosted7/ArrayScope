...
| 11 | **Operations as a platform** ([proposal](proposals/operations-as-a-platform.md), [ADR 0060](decisions/0060-operation-definitions-runtimes-and-discovered-shapes.md)). The custom-operations program (`eaaea1cf`) built the library, manager, and popups; this row makes the operation *set* worth having and every operation readable. Five bundles, in dependency order: **A** native everyday toolbox (magnitude/phase/log-magnitude/normalize/clip/threshold/roll/pad/fractional-resample/median/percentile/std/var/permute/squeeze) and demotion of the pack ops that only wrap NumPy — the sigpy thresholds round-trip float32 → complex128 → float32, and every BART arithmetic op pays a subprocess plus two cfl temp files with a forced complex64 promotion, which on a 4 GiB volume writes the whole array to disk twice to multiply it; numba only where a before/after measures a win. **B** one definition format every op can render, duplicate-to-edit for any op, and deletion of the second parameter editor (`_OperationImportDialog`: 4 columns against the manager's 6, 460 px default against a 430 px minimum leaving room for two parameter rows, and no gallery scenario — which is why a screenshot review pass missed it; its invisible `QComboBox` arrow was app-wide and is fixed in `682cf9ef`). **C** the hidden subprocess bridge (`run_bart` already takes an arbitrary argv) promoted to a user-editable command-template runtime plus named execution environments (interpreter/conda/venv/cwd/env vars, `BART_TOOLBOX_PATH` stops being special-cased), and BART re-expressed as copyable examples. **D** probe-and-cache shape/dtype/windowability adjudication unified with the Tier-2 conformance harness, lifting the `changes_shape` refusal. **E** input slots for ROIs and second arrays, which is what `pics`/`ecalib`/NUFFT actually need. | Per bundle: the all-ops smoke harness covers the new set on float32/complex64; a duplicated built-in yields a user op field-identical to the original; a command-template op runs end-to-end against a fake binary with proven timeout/cancellation; a shape-changing user op survives tiles/LOD without lying to the planner; one real two-input recon runs from the UI. Every bundle that adds a surface adds its gallery scenario in the same bundle. |
