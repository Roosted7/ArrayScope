...
| 3 | **wgpu strangler integration** (ADR 0057; verdict + seed landed — see Done). Ordered slices, each behind tests: **(a)** DONE 2026-07-18 (`d675d57a`): executor grown to the live payload shapes — `BindContentPlanes` multi-plane sessions (GPU flat table/LOD spans rebuilt from the *bound* planes; unbound chunks stay warm in the PageTable, so plane rebind is physically zero-upload), honest pools per representation (scalar `r32float` — zero-imag waste gone; complex `rg32float`; `rgb8` `rgba8unorm` display-ready with levels/LUT bypass), per-pool budgets with same-pool LRU eviction of unpinned only (pinned-full and unbudgeted pools raise loudly), scalar planes ignore complex modes, `DispatchHistogram` over scalar pages; gate-B oracle set extended (multi-plane scroll descriptor-only, warm-rebind zero-upload, scalar/RGB physical truth, per-pool eviction + pins) — tests/gpu 71 passed, none weakened; **(b)** live viewer DONE for montage/complex 2026-07-18 (`50ab831d`, on the MVP below): montages of N scalar tiles and display-ready uint8 RGB tiles + a single complex tile (magnitude/phase/real/imag from `ShaderMapping.component`, phase-color via the canonical cyclic LUT) commit through per-tile `ContentPlane`s with dst rects from the shared `tile_layout_map`; acknowledgement is physical truth per tile (partial residency acks only the resident subset — pinned by a one-layer-pool test) and montage scroll/scroll-back, mode switches, and levels moves are physically zero-upload (`tests/display/test_wgpu_imageview2d.py`; contract suite 33/33 all three backends, nothing weakened; full offscreen suite 2378 passed / 0 failed = baseline 2358 + the 20 new tests). Still loud rejections: complex montage, magnitude-modulated phase color, non-linear scales (log/symlog), non-display-ready/float RGB. 3c prep landed 2026-07-18: LOD-invariant per-plane keys now place coarse payloads in higher-LOD pages behind native-coordinate tile requests, with bound-plane fallback coverage pinned through refinement; the view fallback→refine oracle mirrors the executor oracle. NEW trap (joins the `import_qrenderwidget()` env-stomp trap): any wgpu adapter probe must pin `set_instance_extras(backends=["Vulkan"])` BEFORE its first `request_adapter_sync` — an all-backends instance makes GL adapter enumeration re-init EGL, which SIGABRTs suite workers holding vispy GL state (wgpu-hal panic `gles/egl.rs:305`, root-caused + deterministically reproduced in `8c57a7bf`); *MVP landed 2026-07-18 (scalar 2-D non-montage): `WgpuImageView2D` + `WgpuSurface` behind an explicit settings pin (AUTO never resolves to wgpu); content-keyed residency; executor report is the commit-stats oracle*; **(c)** PREP 2026-07-18: wgpu is registered in the journey harness (15 cells total); the recorded loud rejections report per-row `unsupported`, and the full offscreen ring is 2384 passed / 0 failed (baseline 2378 + 6 new tests). Thomas's real-Wayland 5-row wgpu matrix remains the acceptance gate and was not run here; **(d)** promotion decision by evidence: perf bars vs VisPy on real data, then VisPy retirement review after a release cycle — never a flag-day switch. | Each slice: default-ring tests + real-Wayland journey rows green; promotion gate: journey matrix + perf bars on real data, written verdict in tensor-engine-endpoint.md |
