...
| 12 | **Adjudicate the test-only production backlog** (opened 2026-07-30 by the new commit-time gate `tools/dead_code.py`). The guard reports two classes of production code no shipped path reaches. The *unreachable* class is closed: 54 definitions and 610 lines deleted, including a whole `_cache_status_*` trio orphaned since `b71521fe` and the `frame_controller` preview-helper block orphaned by `6f35667b` (R2 step 6). The *test-only* class — 65 definitions still executing, so every coverage number reads them as healthy — is listed in `_PENDING_ADJUDICATION` in `tests/dead_code.py`, under a ceiling that may only fall (`python tools/dead_code.py --list`). Two patterns account for most of it: **superseded twins**, where a public entry was replaced by an internal one that now carries production (`evaluate_slab` vs `evaluate_slab_from_plan`, `cache_policy.decide_texture_codec` vs the executor's own `_decide_codec_family`, `plugin_operation_ids` vs `discover_plugin_entry_points`) — deleting the twin means migrating its tests onto the survivor, which changes what they exercise; and **a producer whose consumer is still live**, where `render/effects.evaluate_shared_preview` is the only thing that builds shared-preview cohort rows and nothing calls it, while `window/frame_effects.py` still consumes those rows through `_looks_like_shared_preview_rows`, `_admit_preview_cohort_level_evidence` and the `preview-cohort-pending` round-level source. That last one is adjudicated first: it is either rot or a render regression, and 20 tests keep it green either way. | `_PENDING_ADJUDICATION` empty; every entry either deleted with its assertions or given a restored production caller; `_ALLOWLIST` grows only with a stated reason per entry. |
