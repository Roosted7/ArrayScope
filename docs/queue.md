...
| 3 | **Renderer runtime experiments — PULLED FORWARD 2026-07-18** (Datoviz v0.4's external-surface rendering met the graveyard retry condition; G6's shader slices wait on this verdict so they are written once, against the winner, via the backend-neutral command protocol). Two parallel judged vertical slices, SAME scenario and measurements: **(A) Datoviz v0.4-dev** (pinned commit) per the 10-step experiment in [tensor-engine-endpoint](proposals/tensor-engine-endpoint.md) — must include OUR overlay reality (tile-truth overlay, ROI handles, coach marks over the canvas) in the native-child-window matrix, and the PySide6↔PyQt6 bridge question (we run PySide6, not PyQt5 as the scout report assumed); **(B) wgpu-py** comparison slice, same gates. Verdict by the three renderer gates (Qt composition incl. overlays/docks/Wayland; custom shaders + compute incl. a two-pass histogram reduction; upload path + completion/lifetime contract) + journey-matrix non-regression + never-black/physical-truth invariants. | Both slices measured on real Wayland; a written verdict in tensor-engine-endpoint.md; G6 shader work unblocked against the winner |
