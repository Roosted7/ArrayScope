...
| 4 | **G7 — compressed transport — DONE 2026-07-22.** CPU codec + benchmark (`3450b75f`); **Phase A** compressed host-cache RAM win, topology-aware (`6e767001`, ~2.28× working set, 2.0–2.7× large-matrix speedup); **Phase B** native BC/ASTC textures — the discrete transfer + VRAM win via free hardware-sampler decode, with our own CPU + WGSL-GPU BC4 encoder, ASTC on Intel, complex as (real,imag) (`27ac87e9`; A2000 BC 8.0× @ 40.7 dB, Intel ASTC 8.9× @ 42 dB). All default OFF, wgpu render path byte-identical. See the [Done ledger](queue-done.md). **Follow-ups (open):** wire the wgpu atlas/page textures to actually sample BC/ASTC (tile-pipeline integration — Phase B shipped the encoders/policy/benchmark, not the atlas rewrite); BC6H for float tiles. | Measured transfer/VRAM/RAM win on real data, per device topology — MET (RAM + discrete-GPU texture win) |
