...
- `RenderOrchestrator` (`window.render`, composed as `window.renderer`) owns all
  rendering orchestration state: frame planning, montage-session lifecycle,
  presentation commits, prefetch, resource budgets, and every render-path
  timer. It is a `QObject` child of the window, so its timers cannot outlive
  the window. Orchestration code reaches window services via `self.win`; the
