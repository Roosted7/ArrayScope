...
| benchmarks/harness | `profile_montage_workflow`, `rendering_benchmarks`, `profile_scroll_input` + trace tools | per queue-step evidence | `python -m arrayscope.tools.profile_montage_workflow` runs every stage on WGPU and PyQtGraph; pass `--backend {vispy,pyqtgraph,wgpu}` for an explicit single backend (cwd = repo root for `data/` paths) |
