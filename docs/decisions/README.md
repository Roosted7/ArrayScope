...
| [0059](0059-coarse-rung-and-shared-reduced-stage.md) | One coarse rung, fed by a shared reduced-input stage | Accepted and implemented for WGPU raw/FFT and PyQtGraph raw: dynamic complete-scope preview is the explicit product default, one worker/transaction covers 272 tiles, and trace-ACK plus worker-start clauses enforce exclusive preview-before-target order. PyQtGraph composited-complex RGB preview remains an explicit format deferral alongside its pre-existing exact-fill defect. |
