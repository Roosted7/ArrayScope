...
| [0060](0060-operation-definitions-runtimes-and-discovered-shapes.md) | One operation definition, pluggable runtimes, discovered shapes | Proposed. One declarative definition every operation can render (so any op duplicates into an editable user op), runtimes as a named axis (in-process python, python-in-environment, command template generalizing `run_bart`, julia/matlab), execution environments as their own referenced record, and probe-and-cache shape/dtype/windowability adjudication unified with the Tier-2 conformance harness. Supersedes the custom-operations program's refusal of shape-changing user ops. |
