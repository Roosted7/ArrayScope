...
| [0059](0059-coarse-rung-and-shared-reduced-stage.md) | One coarse rung, fed by a shared reduced-input stage | Accepted and implemented. Merges FLOOR/PREVIEW into one rung whose level is a retention decision only, restores the reduced-input evaluation `fbbb6f64` deleted, shares one real-document materialization across the 272-tile FFT montage, and retires the shared-transform scheduler. Five order-balanced WGPU passes preserved full-refined time (5332.1 ms vs 5388.3 ms baseline); the reduced level-4 upload and per-tile coarse-before-exact ACK order are counter/trace pinned. |
