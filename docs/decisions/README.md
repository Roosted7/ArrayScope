...
| [0036](0036-vispy-rendering-backend-experiment.md) | VisPy experiment | Historical experiment; backend retired by 0061. |
| [0037](0037-first-class-vispy-tiled-renderer.md) | VisPy tiled renderer | Historical renderer design; backend retired by 0061. |
| [0038](0038-render-backend-composition.md) | Backend composition | Implemented for WGPU/PyQtGraph; VisPy mechanics in the body are historical. |
...
| [0041](0041-lod-selection-materialization-and-residency.md) | LOD selection/materialization/residency | Implemented via ADR 0050 (async materialization + compatible WGPU/PyQtGraph residency). The three-way split remains the governing contract. |
...
| [0047](0047-auto-image-backend-selection.md) | Capability-probed automatic image backend selection | Superseded in backend choice by 0061; AUTO now selects WGPU when its device gate passes, otherwise PyQtGraph. |
...
| [0050](0050-async-multi-resolution-tile-residency.md) | Async multi-resolution tile residency | Implemented for WGPU tiled scenes with retained preview; PyQtGraph has bounded CPU/item residency; reduce-before-ops consumer and ops-input LOD remain roadmap work. |
...
| [0057](0057-renderer-command-protocol.md) | Renderer command protocol (WGPU backend) | Accepted and implemented; WGPU is the maintained GPU/rendering path with native overlays/glyph text, G6 compute, and bitmap/screen presentation. |
| [0058](0058-canonical-tile-orientation-and-display-transpose.md) | Canonical tile orientation and display-only axis swap | Accepted; implemented for WGPU (shader UV swap) and PyQtGraph (transposed view). |
| [0059](0059-coarse-rung-and-shared-reduced-stage.md) | One coarse rung, fed by a shared reduced-input stage | Core implemented: one real-document reduced materialization feeds all 272 FFT tiles and the shared-transform scheduler is retired. Global coarse-before-refine order remains open: the original per-tile gate allowed 2.1 s of overlap, while routing `DESIRED` through the existing coverage phase fixed order but regressed the three-pass WGPU whole-fill median 23.2%, so that attempt was reverted. |
| [0060](0060-operation-definitions-runtimes-and-discovered-shapes.md) | One operation definition, pluggable runtimes, discovered shapes | Accepted; Bundles A, B, and D implemented, C and E queued. One declarative definition every operation can render (so any op duplicates into an editable user op), runtimes as a named axis, execution environments as their own referenced record, and probe-and-cache shape/dtype/windowability adjudication unified with the Tier-2 conformance harness. Supersedes the custom-operations program's refusal of shape-changing user ops. |
| [0061](0061-retire-vispy-rendering-backend.md) | Retire the VisPy rendering backend | Accepted and implemented; WGPU and PyQtGraph are the maintained backends and VisPy evidence remains historical. |
