...
| LOD | `resident` default for VisPy montage, native-only fallback/menu choice (ADR 0050) | Async pyramid materialization + per-shape-class atlas residency; ingest reduction presents cold tiles at the demanded level (272-tile montage: applied factor 4, estimated tile GPU bytes 280 MB -> 35 MB on the FFT scene, uploads ~0; settle +~15% from worker-side reduction). Level swaps are zero-redundant-work: histogram/level identity is semantic (swap-caused recompute counters must read 0), pyramid levels derive level-from-level, and semantic display-cache stores keep LOD changes from re-running pipelines (per-tile FFT re-run fix). `lod_commuting` capability contract landed; evaluator input reduction is design-noted behind preview-then-refine. Exact inspection stays native. PyQtGraph adoption remains roadmap work. |
