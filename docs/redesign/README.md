...
| [wgpu-pool-layer-leak-2026-07-26.md](wgpu-pool-layer-leak-2026-07-26.md) | 272-tile FFT montage froze at 271/272 on every run: a zero-budget pool orphaned layer 0 at construction, so the complex page pool was permanently one layer short of a budget sized to the exact working set (fixed; first bad `51b826a`). Also records the presentation gate laundering *any* commit exception into the same anonymous stall |
