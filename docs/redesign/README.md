...
| [wgpu-pool-layer-leak-2026-07-26.md](wgpu-pool-layer-leak-2026-07-26.md) | 272-tile FFT montage froze at 271/272 on every run: a zero-budget pool orphaned layer 0 at construction, so the complex page pool was permanently one layer short of a budget sized to the exact working set (fixed; first bad `51b826a`). Also carries the ADR 0051 amendment (§5a) for the presentation gate laundering *any* commit exception into the same anonymous stall |
| [preview-lod-anatomy-2026-07-26.md](preview-lod-anatomy-2026-07-26.md) | What the preview rung costs: a lower/adaptive preview *level* on raw wgpu data **measured and refuted twice** (the cost is commit transactions, and every upload is a native L0 page — instrumented, no reduced page exists to shrink); a 16× input reduction buys only ~2×; and on op pipelines the preview is produced by nobody, falling in the seam between the ladder's *tile-local* test and the shared route's *commuting* one |
| [discarded-rung-evaluation-2026-07-26.md](discarded-rung-evaluation-2026-07-26.md) | 272-tile FFT montage computes and throws away ~8 s of level-1 evaluation per run (first plan made on a pre-settle demand, superseded 155 ms later, uncancellable because one cold tile is one indivisible FFT) plus ~124 duplicate level-2 evaluations. Counter added, **no scheduling changed** — every repair is admission policy that ADR 0046 wants A/B'd |
| [per-commit-transaction-count-2026-07-26.md](per-commit-transaction-count-2026-07-26.md) | Why the preview needs 9 commit transactions and the refinement 1–2: the `min(32, backlog)` item ceiling versus `TileAdmissionQueue`'s free-item bypass, with worker arrival pacing **refuted on the clock**. The transaction is whole-montage by construction (63–69% in a backend republication that iterates every presented payload; empty-delta control 90 ms). **The item ceiling is not the lever — the byte cap is** (−32% stage over 6 order-balanced passes), and "a commit costs the same whatever it carries" is retracted as a 2-tile-regime extrapolation. **The byte cap stays put** — 512 MB is not a byte cap, it is the absence of one. §8 lands what does not depend on that decision: the 2.32× upload undercharge (measured to change nothing here, which is the point), an admission `limit` field so a supply-starved batch stops reading as variance — and a refutation, the "two empty commits" are metadata publications, not no-ops |
| [wgpu-refinement-consumer-price-2026-07-26.md](wgpu-refinement-consumer-price-2026-07-26.md) | What the pixel-identical wgpu refinement actually buys, once the coarse and refined rungs were shown to be the same draw. The level-2 CPU pages **never become wgpu pages** — the renderer binds level-0 native pages for both — so the desired display rung buys no pixels. It does buy two real things through the wrong owner: target-quality summaries that move level evidence from `ROUGH_PREVIEW`/rank 2 to `ROUGH_TARGET`/rank 4, and the ACKs that are currently the only target-settlement token (suppression leaves the session permanently incomplete). A later physical zoom needs **zero uploads in either arm**, so the reduced pages do not warm it either. Recommends splitting evidence, an honest unchanged-binding settlement ACK, and skipping the tile rebind when the physical binding is unchanged; no patch proposed because the split crosses the ADR 0059 ladder work. Scoped to the zoomed-out raw wgpu montage — cropped scroll, zoomed-in single image and PyQtGraph untested |
