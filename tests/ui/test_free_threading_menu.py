...
def _make_window(monkeypatch, free_threaded_build, gil_enabled=False):
    ...
    monkeypatch.setattr(
        free_threading, "interpreter_is_free_threaded", lambda: free_threaded_build
    )
    monkeypatch.setattr(
        free_threading, "gil_currently_enabled", lambda: gil_enabled
    )
...
def test_menu_offers_choices_on_free_threaded_builds(qtbot, monkeypatch):
    ...
    try:
        ...
        auto = next(
            ...
            if a.text() == "Auto-disabled (crashed shortly after launch)"
        )
...
def test_force_disable_persists_setting(qtbot, monkeypatch):
    ...
    try:
        ...
        menu = _performance_submenu(win, "Python Free-Threading")
        action = next(
            a for a in menu.actions() if a.text() == "Force-disabled (GIL on)"
        )
        action.trigger()
        _process_events(qtbot)
        assert (
            win.app_settings.python_free_threading
        ...
        assert (
            win._load_app_settings().python_free_threading
            is FreeThreadingChoice.FORCE_DISABLED
        )
    finally:
        win.close()
...
def test_supervisor_written_auto_disable_shows_checked(qtbot, monkeypatch):
    ...
    try:
        _process_events(qtbot)
        assert (
            win.app_settings.python_free_threading is FreeThreadingChoice.AUTO_DISABLED
        )
        menu = _performance_submenu(win, "Python Free-Threading")
        auto = next(
            a
