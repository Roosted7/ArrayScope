...
def test_auto_compute_policy_caps_tile_fft_workers_at_one():
    ...
    assert policy.montage_tile_workers == 2
...
def test_visible_and_stage_use_resolved_auto_fft_workers():
    ...
    assert policy.fft_workers_visible == 4
    assert policy.fft_workers_stage == 4
    assert policy.fft_workers_for_lane(ComputeLane.VISIBLE) == 4
    assert policy.fft_workers_for_lane(ComputeLane.STAGE) == 4
