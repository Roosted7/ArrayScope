...
class LevelConvergenceStrategy(Protocol):
    def begin(self, tracker: PresentationGenerationTracker, levels, *, source=None, active_tiles=()) -> bool: ...
...
class ProgressiveTileLevelConvergence:
    ...
    def stale_tiles(self, tracker: PresentationGenerationTracker, priority_order=None) -> tuple[int, ...]:
        return tracker.stale_tiles(priority_order=priority_order)

    def acknowledge(self, tracker: PresentationGenerationTracker, *, target_revision: int, accepted_tiles, levels=None) -> None:
...
class UniformLevelConvergence:
    ...
    def acknowledge(self, tracker: PresentationGenerationTracker, *, target_revision: int, active_tiles, levels=None) -> None:
