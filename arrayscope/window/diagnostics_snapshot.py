...
def collect_runtime_diagnostics_snapshot(window) -> WindowRuntimeDiagnostics:
    ...
    montage = MontageRuntimeDiagnostics(
        ...
        waiting_stage_requests=0 if session is None else sum(len(tiles) for tiles in session.stage_fan_in.waiting_tiles.values()),
        ...
        tile_compute_waiting_for_stage=0 if session is None else sum(len(tiles) for tiles in session.stage_fan_in.waiting_tiles.values()),
