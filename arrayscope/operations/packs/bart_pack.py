"""BART command-runtime seam; no operations are registered.

Bundle A removed the unary BART wrappers because native operations now own the
same arithmetic without a subprocess, two cfl temp files, or forced complex64
conversion.  The availability probe, cfl reader/writer, environment handling,
concurrent pipe draining, timeout, and cancellation machinery intentionally
remain intact for Bundle C's general command runtime.  A runnable BART binary
therefore makes this seam available but contributes zero operation specs.
"""
...
def pack_specs() -> tuple:
    """Return no specs; Bundle A replaced every unary BART wrapper natively."""
