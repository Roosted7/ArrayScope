...
def benchmark_montage_tile_payloads(shape=(32, 128, 128), dtype=np.float32) -> BenchmarkResult:
    ...
    plan = make_montage_plan(
        state, axis=0, indices=range(min(8, shape[0])), tile_shape=shape[1:], columns=4
    )
    start = perf_counter()
    payloads = {
    ...
    elapsed = _elapsed_ms(start)
    payload_bytes = sum(
        int(payload.image.nbytes)
        + (0 if payload.histogram_data is None else int(payload.histogram_data.nbytes))
...
def benchmark_roi_stats(shape=(512, 512), dtype=np.float32) -> BenchmarkResult:
    ...
    geometry = RoiGeometry(
        kind="rectangle", rect=(shape[1] * 0.25, shape[0] * 0.25, shape[1] * 0.5, shape[0] * 0.5)
    )
...
def benchmark_large_rgb_montage_histogram_drag(
    shape=(768, 1024), dtype=np.float32
) -> BenchmarkResult:
    ...
    return _result(
        ...
        elapsed,
        output,
        int(base.nbytes + histogram.nbytes),
    )
...
def benchmark_tile_layer_clean_commit(shape=(8, 128, 128), dtype=np.float32) -> BenchmarkResult:
    ...
    sources = {
        int(tile.montage_index): ("montage_tile", int(tile.source_index)) for tile in plan.tiles
    }
    start = perf_counter()
    unchanged = sum(
        1
        for tile in plan.tiles
        if sources.get(int(tile.montage_index)) == ("montage_tile", int(tile.source_index))
    )
    elapsed = _elapsed_ms(start)
    return _result(
        ...
        dtype,
        elapsed,
        np.asarray([unchanged], dtype=np.int64),
        data.nbytes,
    )
...
def benchmark_roi_pan_zoom_stability(shape=(512, 512), dtype=np.float32) -> BenchmarkResult:
    ...
    start = perf_counter()
    values = [
        roi_statistics(roi_values(data[y : y + 256, x : x + 256], geometry)).count
        for x, y in starts
    ]
    elapsed = _elapsed_ms(start)
    return _result(
        "roi_pan_zoom_stability", shape, dtype, elapsed, np.asarray(values, dtype=np.int64), None
    )


def benchmark_offscreen_roi_demand_compute(
    shape=(64, 128, 128), dtype=np.float32
) -> BenchmarkResult:
    data = np.arange(int(np.prod(shape)), dtype=dtype).reshape(shape)
    ...
    return _result(
        ...
        dtype,
        elapsed,
        np.asarray([stats.mean], dtype=np.float64),
        None,
    )
...
def benchmark_fft_stage_warmup_chunked(shape=(24, 24, 17), dtype=np.float32) -> BenchmarkResult:
    coordinator = OperationCoordinator(
        np.zeros(shape, dtype=dtype), operations=(CenteredFFT(axis=2),)
    )
    ...
    chunk_plan = plan_chunked_stage_materialization(
        plan.region_plan, candidate, object(), target_chunk_bytes=4096
    )
    elapsed = _elapsed_ms(start)
...
def benchmark_fft_stage_warmup_unchunked(shape=(24, 24, 17), dtype=np.float32) -> BenchmarkResult:
    ...
    start = perf_counter()
    chunk_plan = plan_chunked_stage_materialization(
        plan.region_plan, candidate, object(), target_chunk_bytes=1024 * 1024 * 1024
    )
    elapsed = _elapsed_ms(start)
    return _result(
...
def benchmark_live_profile_offscreen_unloaded_tile(
    shape=(16, 128, 128), dtype=np.float32
) -> BenchmarkResult:
    data = np.arange(int(np.prod(shape)), dtype=dtype).reshape(shape)
    start = perf_counter()
...
def benchmark_montage_priority_queue_retarget(
    shape=(8, 8, 1024), dtype=np.float32
) -> BenchmarkResult:
    from arrayscope.display.model.tile_priority import MontageTilePriorityQueue, TilePriorityContext

    state = ViewState.from_shape(shape).with_montage_axis(
        2, indices=tuple(range(shape[2])), columns=32, text=":"
    )
    plan = make_montage_plan(
        state, axis=2, indices=tuple(range(shape[2])), tile_shape=shape[:2], columns=32
    )
    full_range = (
        (0.0, float(plan.geometry.columns * (shape[1] + plan.geometry.gap))),
        (0.0, float(plan.geometry.rows * (shape[0] + plan.geometry.gap))),
    )
