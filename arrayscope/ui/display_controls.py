...
class _DimsResizeHandle(QtWidgets.QWidget):
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.QtCore.Qt.MouseButton.LeftButton:
            self._press_y = (
                event.globalPosition().y() if hasattr(event, "globalPosition") else event.globalY()
            )
            event.accept()
...
class DisplayControlBuildMixin:
    ...
    def _create_widget_registry(self, data):
        ...
        self.widgets = {
            "buttons": {
                ...
                "channel": {
                    "complex": QtWidgets.QRadioButton(
                        "complex", enabled=self._current_is_complex()
                    ),
                ...
                "display": {
                    "square_pixels": QtWidgets.QRadioButton(
                        "Square pixels", checkable=True, checked=True
                    ),
                    "fit": QtWidgets.QRadioButton("Fit", checkable=True),
                    "window_relative": QtWidgets.QRadioButton(
                        "Relative", checkable=True, checked=True
                    ),
            ...
            "labels": {
                ...
                "flip": [
                    QtWidgets.QLabel("", alignment=Qt.QtCore.Qt.AlignmentFlag.AlignCenter)
                    for i in range(data.ndim)
                ],
                "shift": [
                    QtWidgets.QLabel("", alignment=Qt.QtCore.Qt.AlignmentFlag.AlignCenter)
            ...
            "spins": {
                "slice_indices": [
                    QtWidgets.QSpinBox(minimum=0, maximum=data.shape[i] - 1)
                    for i in range(data.ndim)
                ]
            },
        }
    ...
    def _register_view_state_bindings(self):
        ...
        self.state_binder.bind(
            ...
            apply=lambda value: (
                channel_buttons[value].setChecked(True) if value in channel_buttons else None
            ),
        ...
        self.state_binder.bind(
            ...
            apply=lambda value: [
                button.setChecked(name == value) for name, button in processing_buttons.items()
            ],
    ...
    def _build_dimension_role_bar(self, data):
        for i, label in enumerate(self.widgets["labels"]["dims"]):
            label.mousePressEvent = lambda event, i=i, label=label: self.dimClicked(event, label, i)
            ...
            label.customContextMenuRequested.connect(
                lambda pos, label=label, dim=i: self._show_operation_context_menu(pos, label, dim)
            )
        ...
        for btn in (
            self.widgets["buttons"]["primary"]
            + self.widgets["buttons"]["secondary"]
            + self.widgets["buttons"]["profile"]
        ):
        ...
        self.dimension_strip.operationRequested.connect(
            lambda axis: self._show_operation_context_menu_for_axis(axis)
        )
        self.dimension_strip.focusedAxisChanged.connect(
            lambda axis: setattr(self, "_focused_dimension_axis", int(axis))
        )
        ...
        self.dims_scroll.setHorizontalScrollBarPolicy(
            Qt.QtCore.Qt.ScrollBarPolicy.ScrollBarAlwaysOff
        )
        ...
        self.state_binder.bind(
            "dimension-strip",
            read=lambda win: (
                tuple(win.data.shape),
                win.view_state,
                tuple(win.profile_axes),
                win.document.current_axes,
            ),
        ...
        for axis in range(data.ndim):
            self.state_binder.bind(
                ...
                apply=lambda value, axis=axis: (
                    None
                    if value is None
                    else self.dimension_strip.update_axis_state(
                        axis, value[0], value[1], value[2], axes=value[3]
                    )
                ),
                on_demand=True,
            )
    ...
    def _build_display_controls_panel(self):
        ...
        for btn in buttons:
            ...
            channel_name = btn.text()
            btn.clicked.connect(
                lambda checked=False, name=channel_name: self._on_channel_clicked(name)
            )
        ...
        for btn in proc_buttons:
            ...
            btn.clicked.connect(
                lambda checked=False, b=btn: self._on_scale_clicked(
                    _scale_value_for_button(self, b)
                )
            )
        ...
        window_absolute.setToolTip(
            "Keep numeric histogram/window limits fixed while changing views"
        )
    ...
    def _build_main_canvas(self):
        ...
        self.img_view = create_image_view(
            ...
            notify=lambda message, timeout=5000: show_status_message(
                self, message, timeout=timeout
            ),
        )
        governor = getattr(self, "resource_governor", None)
        observer = (
            None if governor is None else getattr(governor, "record_gui_callback_observation", None)
        )
        ...
        self.img_view.getView().scene().sigMouseMoved.connect(
            lambda pos: self._on_image_mouse_moved(pos)
        )
        ...
        self.img_view.userLevelsChanged.connect(self._on_display_levels_changed)
        self.img_view.autoWindowRequested.connect(self.auto_window_levels)
    ...
    def _build_header_bar(self, filepath):
        ...
        self.display_toolbar.fitRequested.connect(
            lambda checked=False: self.fit_image_to_view(bool(checked))
        )
        ...
        self.state_binder.bind(
            ...
            read=lambda win: (
                ...
                "absolute"
                if win.widgets["buttons"]["display"]["window_absolute"].isChecked()
                else "relative",
        ...
        pixel_label.statusChanged.connect(
            lambda _text: self.display_toolbar.sync_center_separator()
        )
    ...
    def _build_docks_and_restore_layout(self):
        ...
        self.panel_manager.register_panel(
            ...
            "Profile",
            self.profile_dock,
            default_area=Qt.QtCore.Qt.DockWidgetArea.BottomDockWidgetArea,
        )
        ...
        self.layout_manager.set_managed_dock_visible(
            self.profile_dock, False, reason="initial", preserve_canvas=False
        )
        ...
        self.panel_manager.register_panel(
            ...
            default_area=Qt.QtCore.Qt.DockWidgetArea.LeftDockWidgetArea,
        )
        self.inspection_dock.visibilityChanged.connect(self._on_inspection_dock_visibility_changed)
        self.layout_manager.set_managed_dock_visible(
            self.inspection_dock, False, reason="initial", preserve_canvas=False
        )
        ...
        self.panel_manager.register_panel(
            ...
            "Operations",
            self.operation_dock,
            default_area=Qt.QtCore.Qt.DockWidgetArea.RightDockWidgetArea,
        )
