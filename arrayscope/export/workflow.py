...
class ExportWorkflowMixin:
    def _setup_export_context_menus(self):
        for i in range(self.data.ndim):
            ...
            prim_btn.customContextMenuRequested.connect(
                lambda pos, btn=prim_btn, dim=i: self._show_export_context_menu(pos, btn, dim)
            )
            sec_btn.customContextMenuRequested.connect(
                lambda pos, btn=sec_btn, dim=i: self._show_export_context_menu(pos, btn, dim)
            )
    ...
    def _show_export_context_menu(self, pos, btn, dim):
        ...
        for entry in all_operations():
            ...
            action.triggered.connect(
                lambda checked=False, operation_id=entry.id: self._append_operation(
                    operation_id, dim
                )
            )
    ...
    def _start_export(self, export_dim):
        ...
        if self.singleton[export_dim]:
            QtWidgets.QMessageBox.warning(
                self, "Cannot Export", f"Dimension {export_dim} has size 1 and cannot be exported."
            )
            return

        # Get export settings from dialog
        settings_dialog = VideoExportSettingsDialog(
            parent=self, export_dim=export_dim, data_shape=self.data.shape
        )
        ...
        if settings["format"] == "png":
            ...
            file_path, _ = get_save_file_name(
                self,
                f"Export Video as {settings['format'].upper()}",
                f"export.{settings['format']}",
                file_filter,
            )
            if not file_path:
                return
        ...
        widget_ratio = (
            view.size().width() / view.size().height() if view.size().height() != 0 else 1.0
        )
