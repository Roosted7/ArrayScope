...
def _axes_matching_shape(axes, shape):
    ...
    if len(axes) != len(shape):
        return None
    if any(axis.size != size for axis, size in zip(axes, shape, strict=False)):
        return None
    return axes
...
class PhilipsRECLoader:
    ...
    def _parse_metadata(self):
        ...
        def parse_attribute(attr_elem):
            ...
            if attr_elem.get("ArraySize"):
                values = [
                    float(v) if type_str in ["Float", "Double"] else int(v) for v in text.split()
                ]
                return values
        ...
        for attr in root.findall(".//Series_Info/Attribute"):
            name = attr.get("Name")
            self.general_info[name] = {"Value": parse_attribute(attr), "Type": attr.get("Type")}
        ...
        for img_info in root.findall(".//Image_Info"):
            ...
            for attr in key_elem.findall(".//Attribute"):
                name = attr.get("Name")
                img_meta[name] = {"Value": parse_attribute(attr), "Type": attr.get("Type")}
    ...
    def _setup_dimensions(self):
        ...
        data_types = {img["Type"]["Value"] for img in self.image_infos}
    ...
    def _next_slice(self, fid, img_idx):
        ...
        if self.has_real and self.has_imag:  # R+I
            if img_type == "R":
                self.data.real[
                    :, :, slice_idx, echo_idx, grad_idx, bval_idx, phase_idx, dyn_idx
                ] = scaled_slice
            elif img_type == "I":
                self.data.imag[
                    :, :, slice_idx, echo_idx, grad_idx, bval_idx, phase_idx, dyn_idx
            ...
            if img_type == "M":
                ...
                scale_factor = 1.0
                if (
                    img_meta.get("Contrast Type", {}).get("Value") == "FLOW_ENCODED"
                    and "Phase Encoding Velocity" in self.general_info
                ):
...
class DicomLoader:
    ...
    def load(self):
        ...
        if not hasattr(dcm, "pixel_array"):
            raise ValueError(f"DICOM file does not contain pixel data: {self.dcm_path}")
        ...
        self.metadata.update(
            {
                ...
                "dicom_metadata": {
                    ...
                    "pixel_spacing": list(getattr(dcm, "PixelSpacing", []))
                    if hasattr(dcm, "PixelSpacing")
                    else None,
                    "image_position_patient": list(getattr(dcm, "ImagePositionPatient", []))
...
def _describe_stacking_axis(sidecar_metadata):
    ...
    names = [
        ...
        for metadata in sidecar_metadata
    ]
    ...
    def load(self):
        ...
        with tempfile.TemporaryDirectory(prefix="arrayscope_dicom_") as temp_dir:
            ...
            if len(arrays) == 1:
                ...
                stacking_label, stacking_key, stacking_values = _describe_stacking_axis(
                    sidecar_metadata
                )
                _LOGGER.info(
                    "Dataset stacked automatically along new dimension: %s", stacking_label
                )
                ...
                if stacking_key == "SeriesNumber":
                    series_descriptions = [
                        metadata.get("SeriesDescription") for metadata in sidecar_metadata
                    ]
